...
function [W, funcVal] = Least_CFGLasso_mtl_n(X, Y, rho1, rho2, rho3, NL, opts)
...
elseif opts.init== 0
    W0 = randn(dimension, task_num, task_num);
else
    if isfield(opts,'W0')
        W0=opts.W0;
...
% private functions
    ...
    function [Wp] = FGLasso_projection (W, lambda_1, lambda_2, lambda_3 )
        ...
        for k = 1:size(W, 1)
            v = squeeze(W(k,:,:));
            w = FGLasso_projection_full(v, lambda_1, lambda_2, lambda_3);
            Wp(k,:,:) = w;
        end
    end

    function w = FGLasso_projection_full(v, lambda_1, lambda_2, lambda_3)
        w_1 = zeros(size(v));
        for i = 1:task_num
            sv = v(i,i:min(i+NL,task_num));
            if size(sv, 2) ~= 1
                w0 = zeros(length(sv)-1,1);
                sw = flsa(sv, w0, lambda_1, lambda_2, length(sv), 1000, 1e-9, 1, 6);
                w_1(i,i:min(i+NL,task_num)) = sw';
            else
                if sv(1) > lambda_3
                    sw = sv - lambda_3;
                elseif sv(1) < -lambda_3
                    sw = sv + lambda_3;
                else
                    sw = 0;
                end
        ...
        for k = 1 : size(W, 1)
            for i = 1:task_num
                non_smooth_value = non_smooth_value ...
                    + rho_1 * norm(squeeze(W(k,i,:)), 1) ...
                    + rho_2 * norm(squeeze(W(k,i,:))' * R{i}, 1) ...
                    + rho_3 * norm(squeeze(W(k,i,:)), 2);
            end
%             for j = 1:task_num
%                 non_smooth_value = non_smooth_value ...
%                     + rho_2 * norm(W(k,:,j) * S{j}, 1);
%             end
        end
    end
