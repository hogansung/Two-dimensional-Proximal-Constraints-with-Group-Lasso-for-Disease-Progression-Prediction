...
FD = '../dat/Origin/'
TD = '../dat/TGL_sgl/Longitudinal/'


# mri
mri = read.csv(paste(FD, 'UCSFFSL_02_01_16.csv', sep=''))
mri = mri[, colSums(is.na(mri)) != nrow(mri)] # remove all NA columns
mri = mri[mri$STATUS == 'complete', ] # only complete mri
mri = mri[mri$OVERALLQC == 'Pass', ]
mri = mri[mri$VISCODE == 'sc', ]
drops = c('VISCODE','VISCODE2','EXAMDATE','VERSION','FLDSTRENG','LONISID','LONIUID','IMAGEUID','RUNDATE','STATUS','BASETP1','BASETP2','BASETP3','BASETP4','BASETP5','BASETP6','BASETP7','BASETP8','OVERALLQC','TEMPQC','FRONTQC','PARQC','INSULAQC','OCCQC','BGQC','CWMQC','VENTQC', 'update_stamp','ST8SV')
mri = mri[, !colnames(mri) %in% drops]
...
# write function
writeTGL = function(data, tnames, xname, yname, fnames) {
    for (tname in tnames) {
        y = data[data$VISCODE==tname, yname]
        x = data[data$VISCODE==tname, fnames]
        write.csv(y, paste(TD, xname, '_', yname, '_', tname, '_y.csv', sep=''), quote=FALSE, row.names=FALSE);
        write.csv(x, paste(TD, xname, '_', yname, '_', tname, '_x.csv', sep=''), quote=FALSE, row.names=FALSE);
    }
}
...
tnames = c('m06', 'm12', 'm24', 'm36', 'm48') # 'sc'
mri_mmse = merge(mri_x, mmse)
writeTGL(mri_mmse, tnames, 'MRI', 'MMSCORE', mri_fnames)
meta_mri_mmse = merge(meta_mri_x, mmse)
writeTGL(meta_mri_mmse, tnames, 'META_MRI', 'MMSCORE', meta_mri_fnames)


# adas-cog, start from baseline
adas = read.csv(paste(FD, 'ADASSCORES.csv', sep=''))
tnames = c('m06', 'm12', 'm24', 'm36', 'm48') # 'sc'
mri_adas = merge(mri_x, adas)
writeTGL(mri_adas, tnames, 'MRI', 'TOTAL11', mri_fnames)
writeTGL(mri_adas, tnames, 'MRI', 'TOTALMOD', mri_fnames)
meta_mri_adas = merge(meta_mri_x, adas, by.x = 'RID', by.y = 'RID')
writeTGL(meta_mri_adas, tnames, 'META_MRI', 'TOTAL11', meta_mri_fnames)
writeTGL(meta_mri_adas, tnames, 'META_MRI', 'TOTALMOD', meta_mri_fnames)
