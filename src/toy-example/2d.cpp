    ...
    for (int t = 0; t < 10; t++) {
        ...
        for (size_t i = 0; i < vct.size(); i++) {
            ...
            for (auto& node : vct[i]->nodes) {
                fprintf(stderr, "%f ", node->val); 
            }
            fprintf(stderr, "\n");
        }

        double ssum = calCst(vct, cl);
        double nsum = calCst(vct, l);
